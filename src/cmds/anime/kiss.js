...
module.exports = class KissCommand extends Command {
    ...
    async run(message, args) {
        ...
        if (member.id === message.author.id) return message.reply('can\'t kiss yourself!')
        ...
        const kiss = await fetch('https://nekos.life/api/v2/img/kiss').then(response => response.json());
        //make embed with data provided by api
        const embed = new Discord.MessageEmbed()
            .setTitle(`${member.user.username}, you got kissed!`)
            .setImage(kiss.url)
