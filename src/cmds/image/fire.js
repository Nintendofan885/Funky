...
module.exports = class RipCommand extends Command {
    constructor(...args) {
        super(...args, {
            name: 'fire',
            aliases: ['sf'],
            description: 'Set\'s given user on fire.',
            category: 'Image',
            usage: '[user]'
        });
    }
    ...
    async run(message, args) {
        try {
            ...
            const genImg = await ameApi.generate('fire', { url: member.user.displayAvatarURL({ format: 'png', size: 1024 }) });
            const attachment = new Discord.MessageAttachment(genImg, 'fire.png');
